...
#### 15장) 보안
    ...
    - 시스템의 자원들이 모든 상황 하에서 의도된 대로 사용되고 접근된다면 그 시스템은 안전하다고 말한다.  
    - 보안을 의도적으로 위반하려는 사람들을 가리킬 때 침입자와 크래커라는 용어를 사용한다.  
    - 위협은 취약성의 발견과 같은 보안 위반에 대한 잠재적 가능성인 반면 공격은 보안을 깨뜨리기 위한 시도이다.  
    - 기밀성 침해(Breach of Confidentiality)란 인증받지 않고 자료를 읽는 것을 말한다.  
    - 무결성 침해(Breach of Integrity)란 인증받지 않고 자료를 수정하는 것을 포함한다.  
    - 가용성 침해(Breach of Availability)란 인증받지 않고 자료를 파괴하는 것을 포함한다.  
    - 서비스 가로채기(Theft of Service)란 인증받지 않고 자료를 사용하는 것을 포함한다.  
    - 서비스 거부(DOS, Denial of Serive)란 시스템의 적법한 사용을 막는 것을 포함한다. DDOS 공격이 한 예이다.  
    - 공격자가 보안을 침해하기 위해 가장 일반적으로 사용하는 방법은 통신 상에 있어서 다른 참여자인 것처럼 가장하는 것이다.  
    - 또 다른 일반적인 공격은 주고 받는 데이터를 가로체서 다시 보내는 것이다. 재전송 공격은 정상적인 데이터를 악의적인 의도나 부정한 방식으로 다시 보내는 행위이다.  
    - 중간자 공격은 통신 상의 데이터 흐름 중앙에서 수신자에게는 송신자인 것처럼 가장하고 송신자에게는 수신자인 것처럼 가장하는 것이다.  
    중간자 공격은 활성화된 통신 세션을 가로채는 세션 하이잭킹(Session Hijacking)이 선행된다.  
    - 시스템을 보호하기 위해여 아래 네 가지 수준에서 보안 대책을 수립해야 한다.  
    물리적 수준에서 컴퓨터 시스템을 수용하는 장소는 물리적으로 보호되어야 한다.  
    인적 수준에서 사용자 인증은 적절한 사용자만 시스템에 접근하는 것을 보장하기 위해 신중하게 행해져야 한다.  
    \t피싱과 같은 사회공학적 침입 기법에 대해 방어해야 한다.  
    \t또한 덤스터 다이빙이라 불리는 컴퓨터에 인증 받지 않고 접속하기 위한 정보를 모으는 시도에 대해서 방어해야 한다.  
    - 운영체제 수준에서 시스템은 우연한 또는 고의의 보안 침해로부터 자신을 보호해야 한다. 예를 들어 폭주하는 프로세스가 우연히 서비스 거부 공격을 만들어내거나 패스워드 질의에 대해 노출되지 않도록 해야 한다.  
    - 네트워크 차원에서 데이터 가로채기 등의 침입으로부터 방어해야 한다.  
    - 프로세스는 커널과 함께 작업을 달성하는 유일한 수단이다. 그러므로 보안 침해를 유발하는 프로그램을 작성하거나 정상적인 프로세스가 자신의 행동을 바꾸어 보안 침해를 유발시키는 것이 크래커들의 일반적인 목표이다.  
    예를 들어 원래의 침입 기법이 차단 당하더라도 쉬운 접근을 허용하도록 하는 백도어 디몬을 생성하는 것은 유용한 침입 방법이다.  
    - 자신의 환경을 오용하는 코드 세그먼트를 트로이 목마라고 부른다.  
    트로이 목마의 한 변형은 로그인 프로개름을 흉내 내는 프로그램이다.  
    다른 트로이 목마의 변형은 스파이웨어이다. 스파이웨어의 목표는 사용자의 시스템에 광고를 다운로드해서 보여주거나 특정 사이트를 방문했을 때 팝업 윈도우를 띄우거나 사죵자 시스템 정보를 빼내어 어떤 중상 사이트로 넘기는 것이다.  
    제일 마지막 방식은 은닉채널이라고 알려진 공격의 일반적인 분류로써 은밀한 통신이 발생한다.  
    - 스파이웨어는 최소 권한 원칙 위반의 한 예이다.  
    - 프로그램이나 시스템 설계자가 소프트웨어에 자신만 사용할 수 있는 트랩 문(Trap Door)를 남겨둘 수 있다. 이런 유형의 보안 침해를 영화 War Games 에서 볼 수 있다.  
    - 미리 준비된 인자들이 충족되어 오안 허첨이 발생하면 보안 사고를 일으키는 프로그램을 논리 폭탄이라고 한다.  
    - 스택 오버플로우 또는 버퍼 오버플로우 공격은 네트워크 연결에서 침입 목표 시스템에 인가되지 않은 접근을 얻기 위해 시스템 외부의 침입자가 사용하는 가장 일반적인 방법이다.  
    - 바이러스는 합법적인 프로그램에 내장된 코드 조각이다. 바이러스는 자기 복제가 가능하고 다른 프로그램을 감염시키도록 고안된 프로그램이다.  
    - 바이러스는 목표 기계에 도착하면 바이러스 드롭퍼(Virus Dropper)라고 알려진 프로그램이 바이러스를 시스템에 집어넣는다.  
    - 표준적인 파일 바이러스는 자기 자신을 파일에 덧붙임으로써 시스템을 감염시킨다. 바이러스는 프로그램의 시작 부분을 변형시켜 실행이 시작되면 자신의 코드를 분기하게 만든다.  
    - 부트 바이터스는 시스템의 부트 섹터를 감염시키고 시스템이 부트될 때 운영체제가 읽혀지기 전에 언제나 실행된다.  
    - 매크로 바이러스는 Visual Basic 같은 고수준 언어로 작성된다. 매크로를 실행할 수 있는 프로그램이 실행되면 촉발된다.  
    - 소스코드 바이러스는 소스를 찾고 바이러스를 포함하도록 그것을 수정하고 퍼지도록 도와준다.  
    - Polymorphic 바이러스는 안티바이러스 소프트웨어에 발견되는 것을 피하기 위해 설치될 때마다 변한다. 변화는 바이러스 서명에서만 발생하여 바이러스 기능에 영향을 미치지 않는다.  
    - 암호화된 바이러스는 복호화 코드를 갖고 있어 복호화된 다음 실행된다.  
    - 스텔스 바이러스는 바이러스를 감지하는 시스템 일부를 변형하여 감지를 피한다.  
    - Tunneling 바이러스는 자기 자신을 인터럽트 핸들러 체인에 설치하여 안티바이러스 스캐너의 탐지를 피해간다. 유사하게 디바이스 드라이버에 설치되는 바이러스가 있다.  
    - Multipartite 바이러스는 부트 섹터, 메모리, 파일 등 시스템의 여러 부분을 감염시킨다.  
    - Armored 바이러스는 안티바이러스 연구자가 비밀을 풀고 이해하기 어렵도록 코딩되어 있다.  
    - 브라우저 바이러스는 키보드에 입력되는 모든 것을 기록하는 키스트로크 로거를 포함한 몇 개의 백도어 프로그램을 설치한다.  
    - 공격 표면이란 공격자가 시스템에 침입하기 위해 시도할 수 있는 방법의 집합을 말한다.  
    - Worms 는 시스템 성능에 큰 타격을 주기 위해 산란(Spawn) 기법을 사용하는 프로세스이다. 자신의 복사본을 생성하여 시스템 자원 전부를 소비하게 하여 다른 프로세스의 시스템 사용을 봉쇄할 수 있다.  
    - Worms 는 갈고리(Grapping Hook, 부트스트랩 또는 벡터) 프로그램과 주프로그램으로 이루어져 있다.  
    - 공격받고 있는 시스템에 일단 설치되면 갈고리 프로그램은 자신이 원래 출발했던 시스템에 연결하여 주벌레 프로그램을 갈고리에 걸린 시스템으로 복사한다. 주프로그램은 쉽계 연결할 수 있는 다른 기계를 찾는다.  
    - 포트 스캐닝은 공격이라기보다는 크랙커가 공격 취약점을 찾는 수단이다.  
    - 크래커는 포트 스캐너를 가동 시켜서 포트 25와 같은 특정 포트나 일정 범위의 포트에 접속을 시도한다. 만약 연결이 성공한다면 크래커는 응답 서비스를 통하여 서비스에 결함을 확인할 수 있다.  
    - 서비스 거부 침입은 주로 네트워크 기반으로 이루어진다. 하나의 방법은 너무 많은 시스템 자원을 사용하여 필요한 작업이 실행되지 못하는 것이다. 다른 방법은 한 설비의 네트워크를 못쓰게 한다.  
    - 일반적으로 서비스 거부 침입을 예방하는 것은 불가능하다. 방지 및 알아애기 더욱 어려운 것은 분산 서비스 거부 공격(DDOS, Distributed Denial of Service attacks)이다. 이 공격들은 통상 좀비 컴퓨터에 의해 실행된다.  
    - 사이트는 심지어 자신이 공격당하는 중인지 알 수 없다. 시스템이 느려지는 것이 일시적인 과사용(Surge) 때문인지 해커의 침입으로 인한 공격인지 알아내기 어렵다.  
    - 암호 작성법은 메신지의 송신자와 수신자를 보호하는 데에 사용된다.  
    - 하나의 암화화 알고리즘은 다음과 같은 요소로 구성된다.  
    키의 집합 K, 메시지의 집합 M, 암호문의 집합 C  
    함수 E: K -> (M -> C) 는 주어진 키로부터 임의의 메시지로부터 암호문을 효율적으로 생성하는 함수이다.  
    함수 D: K -> (C -> M) 는 주어진 키로부터 임의의 암호문을 메시지로 효율적으로 생성하는 함수이다.  
    - 대칭 암호화 알고리즘은 암호화 및 복호화에 같은 키를 사용하는 것이다.  
    - 지난 20여 년 동안 미국 민간 응용에서 가장 흔하게 사용된 대칭 암호화 알고리짐은 데이터 암호화 표준(DES, Data Encryption Standard)이다. 몇몇 변환은 블랙박스 변환이다.  
    - DES는 한 번에 청크 단위를 대상으로 동작하므로 블록 암호로 알려져 있다.  
    - 많은 응용의 경우 DES가 이제 안전하지 않은 것으로 간주된다.  
    - 2001년에 NIST는 고급 암호화 표준(AES)이라 불리는 새로운 암호화 알고리즘을 채택하였다.  
    - 다른 대칭 암호화 알고리즘이 널리 사용된다. twofish 알고리즘은 빠르고 간결하며 구현하기 쉽다.  
    - RC4는 가장 일반적인 스트림 암호 중 하나이다. 스트림 암호는 블록보다는 바이트 비트나 비트의 스트림에 대하여 암호화 및 복호화를 실행하도록 설계되었다. 통신 길이가 블록 암호를 사용하면 너무 느릴 정도로 길 경우에 유용하다.  
    - 비대칭 암호화에서는 암호화와 복호화의 키가 다르다. RSA 알고리즘은 암호 공개 키(public key) 알고리즘이며, 가장 널리 사용되는 비대칭 알고리즘이다.  
    - 배디칭 암호화의 사용은 목적지의 공개 키를 공표함으로써 시작된다. 양방향 통신을 위해 발신지도 자신의 공개 키를 공표해야 한다.  
    - 비대칭 암호화는 변환이 아니라 수학적 함수에 기반하고 있어서 계산하는데 훨씬 더 많은 비용이 든다.  
    - 한 메시지의 잠재적 송신자의 집합을 제한하는 것을 인증(Authentication)이라고 한다. 따라서 인증은 암호화를 보완한다.  
    - 첫 번째 형태의 인증 알고리즘은 대칭 암호화를 사용한다. 메시지 인증 코드(MAC, Message Authentication Code)에서 암호화된 체크섬이 메시지로부터 비밀키를 이용하여 생성한다.  
    - 두 번째 타입의 인증 알고리즘은 디지털 서명 알고리즘이다.  
    - 인증은 보안의 여러 면에 참여하는 구성요소이다. 예들 들어 그것은 한 개체가 어떤 행동을 했음에 대한 증명을 제공하는 부인방지(nonrepudiation)의 핵심이다.  
    - 대칭 알고리즘에서 통신 당사자들은 모두 키를 필요로 하고 다른 어느 누구도 그것을 가져서는 안된다. 이것이 비대칭 키 알고리즘을 만들기 위한 노력했던 이유이다.  
    - 불행하게도 공개 키를 분배함에 있어서도 얼마간의 주의가 피룡하다. 공격자도 자신의 나쁜 공개 키를 보낼 수 있다.  
    - 네트워크 프토토콜은 계층으로 구성되며 각 층은 바로 아래층의 클라이언트처럼 행동한다.  
    예를 들어 IP네트워크에서 TCP는 IP의 클라이언트처럼 행동한다.  
    - 암호 기법은 OSI 모델 거의 모든 층에 삽입될 수 있다. SSL은 트랜스포트 층에서 보안을 제공한다. 표준화된 네트워크 층 보안은 인증자의 삽입과 패킷 콘텐츠의 암호화를 허용하는 IP 패킷 포맷을 정의한다.  
    - 사용자의 신원을 인증하는 가장 보편적인 방법이 사용자의 패스워드를 사용하는 것이다.  
    - 침입자가 패스워드를 추측하는 하나의 방법은 사용자를 알거나 사용자에 관한 정보를 갖는 것이다. 다른 방법은 패스워드가 발견될 때까지 가능한 조합을 시도해보는 것이다.  
    - UNIX 시스템은 패스워드 리스트를 은밀하게 유지할 필요성을 피하기 위해 암호화를 이욯한다. 이 방법의 결점은 시스템이 더 이상 패스워드에 대한 제어를 갖지 않는다는 것이다.  
    이같은 이유로 UNIX의 새로운 버전은 패스워드 항들을 수퍼유저만 읽을 수 있는 파일에 저장한다.  
    - 일회용 패스워드는 한 세션이 시작되면 시스템이 무작위로 패스워드 쌍을 선택하여 제시한다.  
    - 컴퓨터 사용의 어떠한 측면에 있어서 보안을 증대시키기 위한 첫 번째 단계는 보안 정책을 갖는 것이다.  
    - 보안 정책이 적절히 구현되었는지 아닌지 판단하기 위해 취약점 평가를 하는 것이다.  
    - 대부분의 취약점 평가의 핵심적 활동은 알려진 취약성에 대해 검사를 실행하는 침입 테스트이다.  
    - 네트워크로 연결된 컴퓨터들은 단독 시스템들보다는 보안 공격에 대해 보다 많이 노출되어 있다.  
    - 침입 탐지는 탐지가 일어나는 시간, 탐지하기 위해 조사되는 입력의 타입들, 대응 능력의 범위 등 다양한 관점의 기술을 포함한다.  
    - 시스템 침입을 탐지하기 위한 설계 공간의 자유로움이 침입 탐지 시스템(IDS, Intrusion Detection Systems)과 침입 방지 시스템(IDP, Intrusion Prevention system)으로 알려진 넓은 범위의 해결책을 낳았다.  
    - IDS 시스템은 침입이 탐지되었을 때 경고를 발생시키는 반면 IDP 시스템은 라우터처럼 동작하며 침입이 탐지되지 않을 경우 트래픽을 통과시킨다.  
    - 서명 기반 탐지는 특정 행위 패턴(또는 서명)을 검사한다. 간단한 예는 여러 번의 로그인 실패를 모니터링 하는 것이다.  
    - 이상 탐지는 컴퓨터 시스템 내에서 비정상적인 행위를 탐지하는 테크닉을 사용한다.  
    - 이상 탐지의 예는 디먼 프로세스의 시스템 호출을 모니터링하여 시스템 호출의 동작이 정상적인 패턴을 이탈하는지 탐지하는 것이다.  
    - 서명 기반 탐지는 위험한 행동을 명시하는 반면 아상 탐지는 정상적인 행위를 명시한다. 서명 기반 탐지는 이미 알려진 공격들을 찾아내고 이상 탐지는 새로운 침입 방법(Zero day 공격)을 알아낸다.  
    
#### 16장) 분산 운영체제
    - 분산 시스템이란 통신 네트워크를 통하여 서로 약하게 결합된 처리기들의 집합니다. 하나의 처리기 관점에서 그 처리기가 갖고 있는 자원을 지역(local)이라 하고 나머지 자원을 원격(remote)라고 한다.  
    - 처리기는 언급되는 문맥에 따라 사이트(site), 노드(node), 컴퓨터, 기계 호스트(host) 등 여러가지 용어로 불린다.  
    - 자원을 가진 쪽을 서버(server)라 하고, 이용하는 쪽을 클라이언트(client) 또는 사용자(user)라고 한다.  
    - 분산 시스템을 구축하는 목적으로는 자원 공유, 연산속도 향상, 신뢰성, 통신 등이 있다.  
    - 분산 시스템의 장점은 다운사이징(downsizing)이라는 새로운 산업화 경향을 만들어냈다.  
    - 네트워크 운영체제가 제공하는 환경에서는 사용자가 원격 자원을 사용하기 위해 원격 호스트에 명시적으로 로그인하거나 원격 호스트에서 자신의 호스트로 데이터를 명시적으로 전송해야 한다. 파일 경로 등이 사용자에게 투명(transparent)하지 못하다.  
    - 분산 운영체제에서는 사용자들이 지역 자원에 접근하는 방법과 동일하게 원격 자원에 접근할 수 있다.  
    - 분산 운영체제에서 데이터 이주는 전체 파일을 전송하는 방법과 파일 중 필요한 부분만을 전송하는 방법이 있다. 만약 두 사이트가 서로 다른 데이터 형태를 사용한다면(리틀 엔디과 빅 엔디안 등) 데이터가 적절히 변형될 필요가 있다.  
    - 분산 운영체제에서 데이터를 이주하는 것보다 연산을 이주하는 것이 효율적일 수 있다.  
    - 분산 운영체제에서 연산 이주의 확장은 프로세스 이주이다. 프로세스 전체 또는 일부를 다른 사이트에서 실행하는 것이 효율적일 수 있다.  
    프로세스 이주는 작업의 균등한 배분, 연산 속도 향상, 하드웨어 선호, 소프트웨어 선호, 데이터 접근 등의 이유로 실행된다.  
    프로세스가 클라이언트로부터 이주되었다는 사실을 숨기거나 명시적으로 허용해야만 이주시킬 수 있도록 시스템을 설계할 수 있다.  
    - LAN(Local Area Networks)은 한 빌딩 또는 몇 개의 인접한 빌딩과 같은 비교적 협소한 지역에 적합하게 설계된 네트워크이다. WAN보다 통신 링크 속도가 빠르고 에러 발생률이 낮은 경향이 있다.  
    - LAN에서 가장 일반적인 링크로 트위스트 페어(twist pair)와 광섬유(fiber optics) 등이 있으며 일반적인 네트워크 구성 형태로 다중 접근 버스(multiclass bus), 링 및 성형(star) 네트워크 등이 있다.  
    - 일반적으로 이더넷이 LAN을 구성하는 데에 이용된다.  
    - WAN(Wide Area Networks)을 구성하는 각 사이트들이 지리적으로 넓게 분산되어 있기 때문에 기본적으로 통신 링크들은 보통 느리고 신뢰성이 낮다.  
    - 호스트들은 보통 LAN에 연결되고, 지역 네트워크를 통해 인터넷과 연결된다.  
    - 네트워크 위상을 구축할 때에 구축 비용, 통신 비용, 가용성 등을 고려하여 구축해야 한다.  
    - 완전 연결 네트워크에서 각 사이트는 모든 다른 사이트에 직접 연결되어 있다. 사이트 수가 늘어날수록 링크의 수도 늘어나 엄청난 구축 비용이 들게 된다.  
    - 부분 연결 네트워크는 전부가 아니라 몇몇 사이트 쌍 사이에 직접 연결이 존재하는 것이다. 그러나 일부 연결의 경우 높은 통신비용(여러 번의 메시지 전달)이 발생한다.  
    - 트리 구조 네트워크는 비교적 구축 비용과 통신 비용이 저렴하다. 그러나 하나의 링크가 고장나면 네트워크가 불할되어 그 사이트의 자식들은 더 이상 다른 사이트와 통신할 수 없다.  
    - 링 네트워크에서는 위와 같은 파티션이 발생하려면 최소 두 링크가 고장나야 한다. 그러므로 원형 네트워크는 트리 네트워크보다 더 높은 가용성을 가지지만 통신 비용이 더 크다.  
    - 성형(스타형) 네트워크에서 각 사이트는 모든 다른 사이트와 최대 두 개의 링크를 통하여 통신할 수 있기 떄문에 통신 비용이 저렴하다. 그러나 중앙 사이트가 고장나면 시스템 내의 모든 사이트가 단절 상태가 된다.  
    - 통신 네트워크 설계자는 다음 네 가지 기본 문제를 고려해야 한다.  
    지명과 이동 변환: 상호 통신하는 두 프로세스의 위치를 어떻게 찾을 것인가?  
    라우팅 전략: 네트워크를 통하여 어떻게 메시지를 보낼 것인가?  
    패킷 전략: 패킷들이 개별적 또는 순서대로 보내지는가?  
    연결 전략: 두 프로세스가 일련의 메시지를 어떻게 보낼 것인가?  
    경쟁: 네트워크가 공유 자원일 때 네트워크 사용 요청의 충돌을 어떻게 해결할 것인가?  
    - 일반적으로 원격 시스템 상에 존재하는 프로세스는 호스트 이름과 식별자에 의해서 식별된다. 호스트 이름은 네트워크 내에서 유일하고, 식별자는 해당 호스트 내에서 유일하다.  
    - 호스트 이름을 지정하기 위한 첫 번째 방법은 모든 호스트는 네트워크를 통해 접근 가능한 다른 모든 호스트들의 이름과 주소를 담고 있는 데이터 파일을 갖고 있는 것이다. 이 방법의 문제점은 호스트를 추가하거나 삭제할 때 모든 호스트의 데이터 파일을 갱신해야 한다는 것이다.  
    - 호스트 이름을 지정하기 위한 두 번째 방법은 정보를 분산시키는 것으로, 정보를 배포하고 가져오기 위한 프로토콜이 있어야 한다. 현재는 이 방법 중 하나인 DNS를 사용하고 있다.  
    - DNS는 이름-주소 변환 뿐 아니라 호스트 이름의 구조도 정의한다. 예를 들어 "bob.cs.brown.edu"는 edu 도메인에 있는 brown 대학교의 cs 학과의 호스트 bob 을 가리킨다.  
    - 일반적으로 시스템은 호스트 이름의 구성요소를 역순으로 분석해 주소로 변환한다.  
    - 가장 흔하게 사용되는 라우팅 전략으로는 고정 라우팅, 가상 라우팅, 동적 라우팅 등이 있다.  
    - 고정 라우팅는 A에서 B까지의 경로가 미리 정해져있는 것이다. 보통 통신 비용을 최소화하기 위해 최단 거리 경로가 지정된다. 설정된 경로가 단절되거나 과부하 상태일 때도 해당 경로를 사용하는 것이 단점이다.  
    - 가상 라우팅은 A에서 B까지의 경로가 한 세션 동안만 고정된다.  
    - 동적 라우팅은 A에서 B까지의 경로가 메시지를 보낼 때마다 결정된다. 일반적으로 메시지 사용률이 가장 낮은 링크를 사용한다. 메시지 전송 순서와 도착 순서가 다를 수 있다. 이는 메시지에 일련 번호를 붙임으로써 해결할 수 있다.  
    - 동적 라우팅은 가장 복잡하지만 복잡한 환경에 가장 적합하다.  
    - 패킷 형태로 구현된 통신은 무연결 메시지 형태로 데이터를 전송한다. 무연결 메시지는 신뢰성이 없다. 대안으로 패킷 수신을 알리는 패킷이 되돌아오는 경우 수신 패킷이 유실될 가능성이 존재하지만 신뢰성이 있다고 할 수 있다.  
    - 연결 전략 중 회선 교환이란 두 프로세스가 통신할 때 고정된 물리 링크가 설정되고 그 링크는 세션 동안 할당되는 것이다. 다른 프로세스는 세션이 종료되지 않으면 해당 링크를 사용할 수 없다.  
    - 연결 전략 중 메시지 교환이란 임시 링크가 하나의 메시지 전송 기간 동안만 설정되는 것이다. 물리적인 링크가 필요할 때마다 동적으로 할당된다.  
    - 연결 전략 중 패킷 교환은 하나의 메시지를 여러 패킷으로 나누어 전송하는 것이다.  
    - 회선 교환은 초기 설정시간이 필요하고 네트워크 대역폭이 낭비되지만 단일 메시지 전송 당 오버헤드는 적다.  
    - 메시지 교환과 패킷 교환은 초기 설정시간이 작지만 단일 메시지 전송 당 오버헤드는 크다.  
    - 패킷 교환은 네트워크 대역폭을 가장 효율적으로 사용하기 때문에 데이터 네트워크에 가장 적합하다.  
    - 네트워크 위상에 따라 둘 이상의 사이트가 하나의 링크에 연결될 수 있다. 
    동시에 그 링크에 정보를 전송하려고 하면 경쟁이 발생할 수 있으며 이런 경우 충돌에 의해 데이터가 손상되기 때문에 해당 정보는 반드시 폐기되어야 한다. 이런 상황을 인지하고 폐기된 정볼르 재전송할 수 있어야 한다.  
    - 이런 경쟁 상황을 방지하기 위해 충돌 탐지 기법(CD, Collision Detection)과 토큰 패싱(token passing)기법이 있다.  
    - CSMA/CD 는 현재 다른 메시지가 링크를 사용하고 있는지 확인(CSMA, Carrier Sense with Multiple Access)하고, 둘 이상의 사이트가 동시에 데이터 전송을 시작하면 충돌 탐지 상황으로 인지하고 전송을 멈춘다.  
    이후 각 사이트는 랜덤 시간이 흐른 후 메시지 재전송을 시도한다. 충돌의 횟수를 제한하는 방법은 네트워크 당 호스트 수를 제한하는 것이다.  
    - 토큰 패싱 기법은 토큰이라는 특수한 메시지가 시스템을 계속해서 순환한다. 정보를 전송하고자 하는 사이트는 토큰이 도착할 때까지 기다려야 하며 토큰이 도착하면 토큰을 없애고 메시지 전송을 시작한다.  
    토큰이 분실되면 시스템들은 토큰 분실을 탐지하고 새로운 토큰을 생성해야 한다.  
    토큰 패싱 네트워크의 장점은 성능을 일정하게 유지할 수 있다는 것이다. 그러나 부하가 적은 네트워크의 경우 CSMA/CD 가 더 효율적일 수 있다.  
    - 네트워크에서 한 시스템의 각 층은 다른 시스템에 존재하는 동등한 층과 통신한다. 각 측은 자신의 고유한 프토토콜을 가질 수 있다.  
    ISO 프로토콜 스택은 아래와 같다.    
    - 물리층은 비트 스트림을 물리적으로 전송하기 위한 기계적, 전기적 사항을 담당한다.  
    - 데이터 링크 층은 오류에 대한 감지와 복구 등 프레임(고정 길이의 패킷) 관련 작업을 담당한다.  
    - 네트워크 층은 연결 및 패킷 라우팅을 담당한다.  
    - 트랜스포트 층은 네트워크로의 하위수준 접근 및 클라이언트 간 메시지 전달을 담당한다.  
    - 세션 층은 세션을 구현하고 프로세스 간 통신 프로토콜을 담당한다.  
    - 프리젠테이션 층은 문자 변환이나 반이중(half duplex), 전이중(full duplex) 모드 등을 포함해 사이트 사이의 포맷 차이 해결을 담당한다.  
    - 응용 층은 사용자들이 직접 교신하는 것을 담당한다.  
    - 메시지는 데이터-네트워크 층으로 내려가 하나 이상의 패킷 단위로 전송된다.  
    - ISO 모델은 그리 폭 넓게 사용되고 있지는 않다. 아마 가장 널리 채택된 프로토콜 스택은 TCP/IP 일 것이다. TCP/IP 프로토콜 스택은 ISO 모델보다 더 적은 층으로 이루어져 있다.  
    - 핸드셰이킹 프로시저를 사용해 링크와 사이트의 결함을 탐지한다. 두 사이트가 직접 연결된 링크를 갖고 있다면 일정 간격으로 두 사이트는 서로에게 메시지를 보낸다.  
      위 메시지를 받지 못했을 때 다른 경로가 존재한다면 다른 경로를 통해 메시지를 보냄으로써 링크 고장과 사이트 고장을 구부할 수 있다.  
      이 때 한 사이트에서 다른 사이트로 메시지가 전달되는 시간을 미리 알 수 없기 때문에 타임아웃 기법이 사용된다.  
    - 만약 타임아웃이 발생했다면 아래 상황 중 하나 이상의 사건이 발생했음을 알 수 있다.  
    1) 사이트가 동작하지 않는다.  
    2) 직접 링크가 동작하지 않는다.  
    3) 대체 경로가 동작하지 않는다.  
    4) 메시지가 분실되었다.  
    - 재구성이란 위 방법을 통해 고장을 탐지한 후에 정상적인 활동을 위해 아래의 과정을 실행하는 것을 말한다.  
    링크가 고장났다면 모든 사이트에 알려 라우팅 테이블을 갱신할 수 있도록 한다.  
    사이트 결함이라면 해당 사이트의 서비스를 이용하지 않도록 한다.  
    - 고장 복구란 고장난 링크 또는 사이트가 복구되었을 때 시스템 중단 없이 시스템에 통합하는 것이다.  
    - 분산 시스템은 일정 수준의 결함을 허용하고 다양한 유형의 결함이 발생했을 때도 정상적으로 계속 동작해야 한다.  
    - 하지만 결함 허용은 어렵고 비용이 많이 든다.  
    - 분산 시스템 설계 시 아래와 같은 것을 고려해야 한다.  
    투명성: 사용자들이 지역 자원에 접근하듯이 원격 자원에 접그할 수 있어야 한다.  
    사용자 이동성: 사용자들이 어떤 시스템에서든지 로그인할 수 있어야 한다.  
    확장성: 시스템이 서비스 부하의 증가에 적응하는 능력을 말한다. 이상적인 개념으로 최대 규모 분산 시스템이 있다. 최대 규모 시스템에서 시스템을 구성하는 하나의 구성 요소의 서비스 요청이 시스템 노드의 총 개수와는 관계없이 상수로 한정되어야 한다.  
    중앙 집중: 중앙 집중식 제어 기법과 집중식 자원은 확장 가능 시스템과 결함 허용 시스템을 구축하는 데는 사용하면 안된다. 현실적으로 어려운 문제이다.  
    - 모든 이더넷 장치는 주소를 지정하기 위해 할당된 유일한 바이트 번호를 갖고 있다. 이를 MAC 주소라 한다.  
    - 한 시스템이 다른 시스템에 데이터를 보내려면 네트워킹 소프트웨어는 목표 시스템의 IP 주소를 포함하는 ARP(Address Resolution Portocol) 패킷을 생성한다.  
    - 효율을 높이기 위해 호스트는 IP-MAC 주소 쌍을 내부 테이블에 캐시한다. 캐시 항목들은 노화(aged)하는데, 시스템에 대한 접근이 주어진 시간 내에 일어나지 않을 때 저장된 항목을 제거하기 위해 필요하다.  
    
#### 17장) 분산 파일 시스템
    - 이상적인 DFS는 클라이언트에게 파일 시스템이 전통적인 중앙 집중형 파일 시스템처럼 보여야 한다. 즉 투명성을 보장해야 한다.  
    - DFS의 원격 접근 서비스에는 부산 구조의 특성에 따른 부담이 수반된다. 송수신 네트워크 지연시간과 통신 프로토콜 처리를 위한 CPU 부하가 그것이다.  
    - 이상적인 DFS는 성능 역시 전통적인 파일 시스템에 필적해야 한다.  
    - 지명(Naming)은 논리 객체들과 물리 객체들 사이의 mapping을 말한다.  
    - 추상화를 이용한 파일 처리의 보다 진보된 단계는 파일 중복을 제공하는 것이다. 하나의 파일 이름이 주어졌을 때 지명은 그 파일 이름에 해당하는 중복 파일의 위치들의 집합을 반환한다.  
    - DFS에서 지명에 관련된 두 가지 주요 개념은 위치 투명성과 위치 독립성이다.  
    위치 투명성(Local Transparency): 파일의 이름이 파일의 물리 저장 위치에 대한 어떠한 정보도 나타내지 않는다.  
    위치 독립성(Location Independency): 파일의 물리 저장장치의 위치가 변경되어도 파일의 이름이 변경되지 않는다.  
    - 파일은 사용자 수준의 텍스트로 된 이름과 시스템 수준의 숫자로 된 식별자를 가진다. 위치 독립적인 지명 기법은 동적 맵핑으로써 동일한 파일 이름을 다른 시점에는 다른 위치로 맵핑한다.  
    따라서 위치 독립성은 위치 투명성보다 더 강한 속성을 가진다.  
    - 기존의 DFS는 사용자 수준의 이름에 대해서는 정적이고 위치 투명한 맵핑을 제공하지만, 파일을 자동으로 이동시키는 파일 이주는 제공하지 않는다. 따라서 위치 독립성은 제공하지 않는다.  
    - 위치 독립성에 의한 데이터와 위치의 분리는 더 좋은 파일 추상화를 제공한다.  
    - 정적 위치 투명성은 사용자에게 편리한 데이터 공유 방법을 제공한다.  
    - 위치 독립성은 저장장치의 계층 구조와 컴퓨터 간 계층 구조로부터 지명의 계층을 분리해준다.  
    - DFS 의 지명 기법에는 세 가지 접근 방법이 있다. 가장 간단한 방법은 파일 이름을 호스트 이름과 로컬 이름의 조합으로 구성하는 것이다.  
    이런 지명 기법은 위치 투명성도 없고 위치 독립성도 없다.  
    - 두 번째 방법은 Sun의 네트워크 파일 시스템(NFS)에 의해 대중화된 방법이고, 마지막 방법은 하나의 전역 이름 구조가 시스템 내의 모든 파일에 적용된느 것이다.  
    - 지명 구조를 평가하는 가장 중요한 기준은 관리의 복잡성이다.  
    - 투명성을 제공하는 지명 기법을 구현하기 위해 먼저 파일 이름을 대응하는 위치로 맵핑하는 일이 선행되어야 한다.  
    - 중요한 맵핑 정보의 가용성을 증가시킬 목적으로 중복, 로컬 캐싱, 또는 두 방법 모두를 사용할 수 있다.  
    - 저수준의 식별자를 구현하는 일반적인 방법은 구고적 이름을 사용하는 것이다. 이런 이름은 보통 두 부분으로 구선된 비트 스트링이다.  
    첫 번째 부분은 파일이 속해 있는 구성 단위를 식별하고 두 번째 부분은 단위 내의 특정 파일을 식별한다.  
    - 캐싱은 대상 데이터가 로컬 메모리에 캐시되어 있지 않으면 데이터 복사본을 서버에서 클라이언트로 이동시키고 접근은 캐시된 복사본 상에서 지역적으로 행히진다.  
    - 캐시를 사용함으로써 파일의 사용이 항상 서버로의 네트워크 교통을 유발하는 것은 아니다.  
    - 캐시된 복사본이 변경되면 변경된 내용은 마스터 복사본에 반영시켜 일관성을 유지해야 한다.  
    - 캐싱 단위의 크기가 클수록 적중률은 증가하지만 실제 데이터 전송 시간이 지연되고 일관성 문제가 심각해진다.  
    - 메모리 캐시는 디스크 없는 워크스테이션을 허용한다.  
    - 메모리 캐시는 디스크 캐시보다 더 빨리 접근될 수 있다.  
    - 캐싱 갱신 정책 중 하나는 데이터가 어떤 캐시에서 수정되는 순간에 디스크에 기록하는 것이다. 이와 같은 바로 쓰기의 장점은 신뢰성이다. 반면 읽기 시는 캐싱 이점을 누릴 수 있으나 쓰기 시 캐싱의 이점을 누릴 수 없다.  
    - 대안으로 후위 쓰기 캐싱으로도 알려진 지연쓰기 전략이 있다. 변경된 데이터는 캐시에 쓰이가 나중에 서버로 보내진다.  
    - 변경된 데이터 블록이 서버에 반영되는 시점의 차이에 따라 여러 변형된 지연 쓰기 기법들이 있다.  
    - 한 가지 방법은 클라이언트 캐시에서 쫓겨나느 블록을 서버로 보내 복사본에 반영시키는 것이다.  
    - 다른 방법은 캐시를 정기적으로 검사하여 최근 조사 이후에 갱신된 모든 블록을 서버에 반영시키는 방법이다.  
    - 일관성 측면에서 캐시된 데이터가 가장 최신 데이터라는 것이 보장되어야 한다. 캐시 데이터의 유효성을 검증하기 위해서 클라이언트 주도 방식이나 서버 주도 방식으로 검증할 수 있다.  
    - 클라이언트 주도 방식은 클라이언트가 서버와의 접촉을 시도해서 로컬 데이터가 마스터 복사본과 일치하는지 검사한다. 검사 빈도가 이 방식에서 가장 중요한 요소이며 일관성의 강약을 결정한다.  
    - 서버 주도 방식에서 서버는 각 클라이언트가 캐시한 파일에 대한 정보를 기록한다. 서버는 잠재적인 불일치를 감지했을 때 대응을 시작한다.  
    - 캐싱과 원격 서비스 중 어떤 방식을 선택할 지는 성능과 단순성의 절충에 달려있다.  
    - 파일 접근 패턴에서의 높은 지역성은 캐싱을 더욱 효과적으로 만든다. 결과적으로 서버 부하와 네트워크의 통신량은 감소되며 규모 가변성은 증가한다.  
    - 대량의 데이터를 한 번에 전송할 때(캐싱의 경우) 네트워크 부담이 개개의 접근 요청에 대한 소량의 데이터를 여러 번 전송할 때(원격 서비스의 경우)의 네트워크 부담보다 적다.  
    - 캐시 일관성 문제는 캐싱의 가장 큰 단점이다.  
    - 캐싱은 로컬 디스크 또는 큰 메모리를 가진 시스템에 적합하다.  
    - 원격 서비스 패러다임은 네트워크 전체를 아우르는 로컬 파일 시스템 인터페이스의 확장이라 할 수 있다. 따라서 기계 간 인터페이슨느 사용자 인터페이스와 일치한다.  
    - 상태형 파일 서비스(Stateful File Service) 에서 서버는 자신의 디스크로부터 파일에 대한 정보를 가져오고 이것을 자신의 메모리에 저장한다.  
    그리고 클라이언트에게 클라이언트와 클라이언트가 연 파일에 대해 고유한 연결 식별자를 배정한다. 식별자는 그 세션이 끝날 때까지 그 파일로의 접근을 위해 사용된다.  
    - 무상태형 파일 서비스에서는 각 요청이 더 많은 정보를 제공하게 함으로써 서버 쪽에서는 상태 정보를 유지할 필요가 없다.  
    효율성 때문에 열린 파일에 대한 테이블을 유지하기는 하지만 무상태형 파일 서버에서는 열린 파일에 대한 테이블을 유지할 필요는 없다.  
    - 무상태형 서비스 사용의 단점은 요청 메시지가 길고 요청에 대한 처리가 느리다는 점이다.  
    - 여러 기계에 파일을 중복시키는 것은 전체 시스템의 가용성을 증가시킨다.  
    - 사용자에게 중복에 대한 세부적인 내용을 숨기는 것이 바람직하다. 이를 위해서 중복된 파일의 이름을 특정 복사본으로 맵핑시키는 지명 기법이 필요하다. 복사본이 있다는 사실을 고수준에서는 알지 못하지만 저수준의 이름들에 의해서는 서로 구분되어야 한다.  
    - 복사본과 관련되어 발생하는 문제점은 복사본을 갱신하는 것이다.  
    많은 비용을 들여 일관성을 유지하거나 프로세스의 계속적인 진행을 보장하기 위해 일관성을 희생시키는 방법이 있다.  
    
#### 18장) 분산 동기화
    - 중앙 집중식 시스템에서는 하나의 공유 메모리와 클록을 사용하므로 두 개의 사건이 발생해도 그 순서를 결정하는 것이 가능하다.  
    - 분산 시스템에서는 공유 메모리와 공유 클록이 존재하지 않기 때문에 두 사건의 순서를 결정하기가 불가능한 경우가 있다.  
    - 사건의 전후 관계는 분산 시스템에서 부분적인 순서만을 정의한다. 전체 사건의 순서를 정하기 위해 분산 알고리즘이 필요하다.  
    - 만약 두 사건이 전후 관계가 없다면 이 두사건은 병행(Concurrently)으로 실행되었다고 한다.  
    - 사건 A 가 B보다 이전에 발생했는지 결정하는 알고리즘 중 하나는 다음과 같다.  
    1) 각 시스템 사건에 하나의 타임스탬프를 연관시킨다. 그러면 전역 순서화 조건을 다음처럼 정의할 수 있다.  
    2) 만약 모든 사건 쌍 A 와 B 가 A 가 B 보다 선행된다면 A의 타임스탬프가 B의 타임스탬프보다 무조건 작아야 한다.  
    3) 이를 위해 각 프로세스 내의 논리 클록을 정의한다. 논리 클록(LC)은 간단한 카운터로 사건이 발생할 때마다 증가한다.  
    4) 사건 A 가 사건 B 보다 먼저 발생한다면 LC(A) < LC(B) 가 성립한다. 이 기법은 동일한 프로세스 내 임의의 두 사건에 대해 전역 순서화 조건을 만족시킬 수 있다.  
    5) 서로 다른 프로세스의 경우 그 값이 다를 수 있다.  
    6) 그런 경우 순서화 조건에 맞추어 메시지를 수신할 때에 논리 클록 값을 변경하면 된다. 예를 들어 A -> B 이고 LC_i(A) = 200, LC_j(B) = 195 이면 j 프로세스의 논리 클록 값을 201 로 변경한다.  
    - 중앙 집중식 방식에서는 하나의 프로세스가 임계 영역으로의 진입을 조정하는 역할을 전담하도록 함으로써 상호 배제를 구현한다.  
    임계 영역을 실행 중인 프로세스가 없으면 조정자는 응답 메시지를 바로 보낸다. 그렇지 않으면 요청은 메시지 큐에서 대기해야 한다.  
    이 알고리즘은 상호 배제를 보장한다. 또한 조정자의 스케쥴링 정책이 공정하다면 기아 상태는 발생하지 않는다.  
    - 완전 분산형 접근에서 특정 프로세스가 임계 영역에 들어가고자 할 때 그 프로세스는 새로운 타임스탬프를 생성하고, 자신을 포함한 시스템 내의 모든 프로세스에게 메시지를 보낸다.  
    메시지를 받은 프로세스는 즉시 응답을 하거나 또는 자신이 이미 임계 영역 안에 있으면 응답을 보류한다.  
    시스템 내의 다른 모든 프로세스로부터 응답 메시지를 받은 프로세스만이 임계 영역으로 진입할 수 있다.  
    - 각 프로세스는 아래 세 가지 항목에 따라 응답 메시지를 즉시 보낼지 보류할지 결정한다.  
    1) 프로세스가 임계 영역 안에 있으면 응답을 보류한다.  
    2) 프로세스가 임계 영역으로의 진입을 원하지 않으면 응답 메시지를 보낸다.  
    3) 프로세스가 임계 영역으로 진입하려 하지만 아직 진입하지 못한 경우 요청 메시지의 타임스탬프와 자신의 타임스탬프를 비교한다.  
    자신의 요청 타임스탬프가 더 크면 요청을 보낸 프로세스가 더 먼저 요청한 것이므로 응답을 보내고 그렇지 않으면 응답을 보류한다.  
    - 위 방식은 상호 배제를 보장하며, 교착 상태 발생을 방지하며, 타임스탬프 순서화(FCFS) 를 사용하므로 기아 상태가 발생하지 않으며, 최소의 메시지 교환을 필요로 한다.  
    - 반면 위 방식은 각 프로세스가 다른 모든 프로세스 정보를 알아야 하며, 만약 프로세스 중 하나라도 고장이 발생하면 전체 알고리즘이 파괴된다.  
    또한 임계 영역에 진입한 적 없는 프로세스도 임계 영역에 들어가려는 다른 프로세스를 위해 자신의 동작을 자주 정지시켜야 한다.  
    따라서 이 프로토콜은 협동하는 프로세스 수가 적고 안정적일 때 적합하다.  
    - 상호 배제를 제공하는 또 다른 방법은 토큰 패싱 기법이다. 토큰이란 시스템 내에서 전달되는 일종의 특수한 메시지이다. 토큰의 소유주가 임계 영역에 진입할 수 있다.  
    - 경쟁이 심한 경우 한 번 출입할 때마다 하나의 메시지가 필요하다. 반면 경쟁이 심하지 않을 때는 한 번 출입할 때마다 많은 메시지가 필요하다.(여러 번의 토큰 패싱)  
    - 토큰이 분실되면 하나의 프로세스를 선택하여 그 프로세스가 새로운 토큰을 생성하도록 해야 한다.  
    - 만약 임의의 프로세스에 고장이 발생하면 새로운 토큰을 생성하도록 해야 한다.  
    - 분산 시스템을 집중식 시스템과 비교할 때 트랜잭션의 원자성을 보장하는 것이 훨씬 복잡하며 여러 사이트가 단일 트랜잭션의 실행에 참여하고 있기 때문에 어려움이 발생한다.  
    - 분산 시스템의 트랜잭션 조정자 기능은 분산 시스템에 있는 여러 트랜잭션들의 실행이 원자성을 유지하도록 보장하는 것이다.  
    - 원자성을 보장하기 위해 트랜잭션 T를 실행하고 있는 모든 사이트들이 실행의 최종 결과에 동의해야 한다.  
    - 이러한 속성을 보장하기 위하여 T의 트랜잭션 조정자는 확약(commit) 프로토콜을 실행한다.  
    - 가장 간단하고 많이 사용되는 확약 프로토콜은 2단게 확약(2PC, 2 Phase Commit Protocol) 프로토콜이다.  
    - T는 사이트 Si 에서 시작한 트랜잭션이고, Ci 는 사이트 Si에 있는 트랜잭션 조정자라고 하자.  
    - T가 실행을 완료할 때 즉 T를 실행한 모든 사이트들이 T가 완료되었다는 것을 Ci에게 알려주었을 때 Ci는 2PC 프로토콜을 시작한다.  
    1) Ci 는 레코드 prepare T 를 로그에 추가하고, 안정된 저장장치에 레코드를 기록한다. 그 다음 T를 실행한 모든 사이트에 prepare T 메시지를 전송한다.  
    2) 메시지를 전송 받은 해당 사이트의 트랜잭션 관리자는 자신의 T 부분에 대해 확약할 것인지 결정한다. 확약할 수 없으면 레코드 no T 를 로그에 추가하고 abort T 메시지를 Ci 에 전송한다.  
    3) 응답이 예이면 ready T 를 로그에 추가하고 T 에 대응하는 모든 러코드들을 안정된 저장장치에 저장한다. 그 다음 트랜잭션 관리자는 ready T 메시지를 Ci 에 전송한다.  
    4) Ci 가 모든 사이트로부터 메시지에 대한 응답을 받거나 특정 시간 내에 응답이 없을 때 Ci 는 트랜잭션 T 를 확약 혹은 거부할 것인지 결정한다.  
    5) 모든 사이트로부터 ready T 를 수신했을 때 확약되고, 그렇지 않으면 거부된다. 답신에 따라 commit T 또는 abort T 를 로그에 추가하고 안정된 저장장치에 저장한다.  
    6) Ci 는 commit T 또는 abort T 메시지를 관련 사이트에 전송하고 각 사이트는 메시지를 받은 후 로그에 기록한다.  
    - ready T  메시지는 조정자가 명령한 commit T 또는 abort T 를 따르겠다는 한 사이트의 약속이다.  
    - 2PC 프로토콜에서 한 사이트 Sk 가 고장으로부터 복구될 때 고장 발생 시 실행 중이던 트랜잭션의 운명을 결정하기 위해 관련된 로그를 조사해야 한다.  
    1) 로그가 commit T 레코드를 가질 경우 해당 사이트는 redo T 를 실행한다.  
    2) 로그가 abort T 레코드를 가질 경우 해당 사이트는 undo T 를 실행한다.  
    3) 로그가 ready T 를 가질 경우 Ci 에 문의하여 commit T 인지 abort T 인지 확인하다.  
    4) Ci 동작하지 않아 문의할 수 없는 경우 시스템에 있는 모든 사이트에 query status T 메시지를 각 사이트가 commit T 를 받았는지 abort T 를 받았는지 확인한다.  
    5) 모든 사이트가 확약에 대한 정보를 제공하지 않으면 확약 여부에 대해 알 수 없으므로 해당 트랜잭션을 연기한다. 다른 사이트에 주기적으로 질의 메시지를 재전송한다.  
    6) 로그가 T 에 관한 어떤 제어 레코드(abort, commit, ready) 도 가지고 있지 않은 경우 Ci 는 응답을 받지 못했을 것이므로 abort T 메시지를 기록했을 것이다. 따라서 undo T 한다.  
    - 2PC 프로토콜에서 조정자가 공나면 참여 사이트들이 트랜잭션 T 의 상태를 결정해야 한다.  
    1) 만약 활성 사이트가 자신의 로그에 commit T 를 포함하고 있으면 T 는 확약되어야 한다.  
    2) 만약 활성 사이트가 자신의 로그에 abort T 를 포함하고 있으면 T 는 취소되어야 한다.  
    3) 만약 어떤 활성 사이트가 자신의 로그에 ready T 를 포함하고 있지 않으면 결함이 발생한 조정자 Ci 는 T 를 확약하도록 결정할 수 없었을 것이다. 이 경우 T 를 취소하는 것이 바람직하다.  
    4) 모든 사이트가 ready T 를 갖고 commit T 또는 abort T 를 갖지 않는 경우 활성 사이트들은 Ci 가 복구되기를 기다려야 한다. T 의 상태가 결정되지 않았기 때문에 T 는 계속 시스템 자원들을 유지한다. 이러한 상황을 블록킹 문제라고 한다.  
    - 2PC 프로토콜에서 네트워크가 고장났을 경우 조정자 관점에서 보면 다른 사이트들에 고장이 발생한 것으로 보인다. 따라서 위 방법들이 네트워크 문제에도 적용된다.  
    - 각 트랜잭션 관리자는 복구를 위해 로그를 유지해야 하고, 관련 사이트에서 실행되는 트랜잭션들의 병행 실행을 조정하기 위해 적절한 병행성 제어 방법을 채택해야 한다.  
    - 락 프토토콜은 분산 환경에서도 적용될 수 있다. 단지 변경될 부분은 락 관리자를 구현하는 방법 뿐이다.  
    - 시스템에 어떤 중복 자료도 없을 경우 각 사이트는 자신의 사이트에 저장되어 있는 자료 항목들에 대해 락과 언락 요청을 관리하기 위해 로컬 락 관리자를 갖는다.  
    이 방법의 장점은 구현이 간단하다는 것이다. 그러나 교착 상태 취급이 복잡하다.  
    - 단일 조정자 방식은 특정 사이트에서 단일 락 관리자를 갖는 것이다. 이 방법은 구현이 단순하고 교착 상태 취급도 단순하다.  
    반면 락 사이트는 모든 요청을 처리해야 하기 때문에 병목이 발생할 수 있다. 또한 락 사이트가 고장나면 병행성 제어가 불가능하여 복구가 요구된다.  
    - 다수결 프로토콜에서 시스템은 각 사이트에 락 관리자를 유지한다. 각 관리자는 자신의 사이트에 저장되어 있는 모든 자로와 복사본에 대한 락을 관리한다.  
    어떤 트랜잭션이 n 개의 다른 곳에 복사되어 있는 자료 항목 Q 를 락하고자 할 때 Q 가 저장되어 있는 n 사이트 중 1/2 이상의 사이트에 락 요청 메시지를 전송한다.  
    각 락 관리자는 락이 즉시 허락될지 결정한다.  
    - 다수결 프로토콜은 구현하기 복잡하고, 교착상태 알고리즘이 변경되어야 한다. 그리고 단지 하나의 자료 항목이 락 상태가 될 경우에도 교착 상태가 발생할 수 있다.  
    - 편향된 프로토콜은 다수결 프로토콜과 유사하나 차이점은 공유 락 요청이 배타적 락 요청보다 선호된다는 것이다.  
    트랜잭션이 공유 락을 요청할 경우 Q 의 복사본을 갖고 있는 한 사이트의 락 관리자로부터 Q 에 관한 락을 요청한다.  
    반면 트랜잭션이 배타적 락을 요청할 경우 Q 의 복사본을 갖는 모든 사이트의 락 관리자로부터 락을 요청한다.  
    읽기 연산이 쓰기 연산에 비해 많은 경우 상당한 장점이 있다. 그러나 쓰기 동작에 대해 추가적인 부담이 존재한다.  
    또한 교착상태 취급에 있어 다수결 프로토콜의 단점인 복잡함을 공유한다.  
    - 주 복사본 방법은 복사본들 중 한 개를 주 복사본으로 선정하는 것이다.  
    중복된 자료에 대한 병행성 제어를 중복되지 않은 자료의 병행성 제어와 유사한 방법으로 취급할 수 있다.  
    반면 주 사이트가 고장나면 복사본을 갖고 있는 다른 사이트가 있다 하더라도 접근할 수 없다는 단점이 있다.  
    - 타임스탬프의 기본 아이디어는 각 트랜잭션이 순차적인 순서를 결정하는 데에 사용되는 유일한 타임스탬프를 갖는 것이다.  
    - 유일한 타임스탬프를 생성하기 위한 기본적인 두 가지 방법에는 집중형과 분산형이 있다. 집중형에서는 타임스탬프를 분배하기 위해 단일 사이트가 사용된다.  
    - 분산형에서 각 사이트는 논리적인 카운터 혹은 로컬 클록을 사용하여 유일한 로컬 타임스탬프를 생성한다. 전역의 유일한 타임스탬프는 유일한 로컬 타임스탬프와 유일한 사이트 식별사를 접속함으로써 얻어진다.  
    - 공정한 타임스탬프 생성을 위해 각 사이트 Si 내에 로컬 타임스탬프를 생성하는 논리 클록(LC) 을 정의한다.  
    - 다수의 논리 클록이 동기화되는 것을 보장하기 위하여 사이트 Si 는 타임스탬프 (x, y) 를 가진 트랜잭션 T 가 자신의 사이트를 방문하고, x 가 LC의 현재 값보다 클 때마다 자신의 논리 클록을 증가시킨다.  
    - 연속 롤백 현상을 제거하기 위하여 기본 타임스탬프 기법과 2PC 프로토콜을 결합하여 직령성(serializability) 를 보장하는 프로토콜을 얻을 수 있다.  
    - 시스템 자원들 간의 전역 순서화를 정의함으로써 자원 순서화 교착 방지 기법을 분산 환경에 적용할 수 있다.  
    - 시스템 내의 프로세스 중 하나를 은행원 알고리즘을 실행하는 데에 필요한 정보를 유지하는 프로세스로 지정함으로써 은행원 알고리즘을 분산 시스템에 적용할 수 있다.  
    - 모든 자원 요청은 해당 프로세스를 통해서 실행된다. 전역 자원 순서화 교착상태 방지 기법은 구현이 쉽고 오버헤드가 거의 없다. 반면 은행원 알고리즘은 오버헤드가 크다.  
    - 자원 선점형 타임스탬프 순서화 방식에 기반을 둔 새로운 교착 방지 기법을 소개한다.  
    - 선점을 제어하기 위해 각 프로세스에게 고유의 우선순위를 부여한다. Pi 가 Pj 보다 높은 우선순위를 갖고 있으면 Pi 는 Pj 를 기다리고 그렇지 않으면 Pi 는 롤백한다.  
    이 기법의 한 가지 단점은 기아 상태가 발생할 수 있다는 점이다. 매우 낮은 우선 순위를 가진 프로세스들은 계속해서 롤백될 수 있다. 이 문제는 타임스탬프를 사용함으로써 해결할 수 있다.  
    타임스탬프를 이용한 교착 방지 기법은 다음 두 가지로 분류된다.  
    - wait-die 기법: Pi 가 Pj 보다 더 작은 타임스탬프를 가진 경우에만 기다리는 것이 허용된다. 그렇지 않은 Pi는 롤백한다.  
    - wound-wait 기법: Pi 는 Pj 보다 큰 타임스탬프를 가진 경우에만 기다릴 수 있다. 그렇지 않으면 Pj 를 롤백한다.  
    두 기법 모두 롤백되는 프로세스에 새로운 타임스탬프를 다시 할당하지 않으며 따라서 기아 상태를 방지할 수 있다.  
    타임스탬프는 항상 증가하므로 롤백되는 프로세스는 결국 가장 작은 타임스탬프를 갖게 되어 다시 롤백되지 않는다.  
    - wait-die 기법에서 오래된 프로세스는 새로운 프로세스가 자원을 방출하기를 기다려야 한다. 따라서 프로세스가 오래될수록 더 많은 시간을 기다리는 경향이 있다. 또한 프로세스 실행 전 여러 번 롤백될 수 있다.  
    - wound-wait 기법에서는 오래된 프로세스가 결코 새로운 프로세스를 기다리지 않는다. 또한 wound-wait 기법은 롤백하는 경우가 적게 발생한다.  
    - 교착상태 방지 알고리즘은 교착상태가 발생하지 않았음에도 자원을 선점할 수 있다. 불필요한 선점을 방지하기 위해 교착상태 탐지 알고리즘을 사용할 수 있다.  
    - 분산 시스템에서 교착상태 탐지를 위해 고려되어야 하는 중요한 문제는 대기 그래프를 어떻게 유지하는가 하는 것이다.  
    - 어떤 로컬 대기 그래프가 사이클을 가지면 교착상태가 발생했음을 의미한다. 그러나 역으로 로컬 대기 그래프에 사이클이 없다는 사실이 교착상태가 없다는 것을 의미하지 않는다.  
    - 교착 상태가 없음을 보이기 위해서는 시스템의 모든 로컬 대기 그래프의 합집합에 사이클이 없음을 보여야 한다.  
    - 중장 집중형 접근 방식에서 전역 대기 그래프가 모든 로컬 대기 그래프의 합으로 이루어진다. 전역 대기 그래프는 교착 탐지 조정자 역할을 담당하는 하나의 프로세스에 의해 유지된다.  
    - 실제 그래프는 완전한 관찰자에 의해 보여지는 시스템의 실제 상태를 의미한다. 반면 구축 그래프는 알고리즘을 실행하는 동안 조정자에 의해 생성된 근사 그래프이다.  
    - 대기 그래프가 구성되는 시점에 따라 다음 세 가지 방법이 있다.  
    1) 로컬 대기 그래프에 새로운 간선이 삽입되거나 제거될 떄마다  
    2) 주기적으로 대기 그래프에 많은 변화가 일어났을 때마다  
    3) 조정자가 사이클 탐지 알고리즘을 실행할 필요가 있을 때마다  
    - 교착상태 탐지 알고리즘이 실행되면 조정자는 전역 그래프를 탐색한다. 만약 사이클이 발견되면 롤백될 희생자가 선택된다.  
    - 어떤 기법을 사용하더라도 다음 두 가지 상황으로 인하여 불필요한 롤백이 발생할 수 있다.  
    1) 거짓 사이클이 전역 대기 그래프에 존재하는 경우  
    2) 실제 교착상태가 발생하여 희생자가 선택되었지만 교착상태와 무관한 이유로 연관 프로세스가 중지되어 롤백할 필요가 없어질 경우  
    - 완전 분산형 교착상태 탐지 알고리즘에서 모든 사이트가 전체 그래프의 일부를 표현하는 대기 그래프를 동적으로 구성한다.  
    - 각 사이트는 자신의 로컬 대기 그래프를 유지한다. 이 로컬 대기 그래프는 Pex 라는 특수한 노드를 포함한다는 점에서 앞에서 언급한 로컬 그래프와 다르다.
    - 분산 알고리즘들은 시스템 내의 다른 프로세스들과의 공동 작업을 위해 필요한 기능을 담당하는 조정자 프로세스를 갖고 있다.  
    - 사이트의 고장으로 조정자 프로세스의 실행이 불가능해지면 다른 사이트에 조정자 기능을 이전시킴으로써 시스템을 계속 작동시킬 수 있다. 조정자 기능을 이전할 사이트를 결정하는 알고리즘을 선출 알고리즘이라고 한다.  
    - 선출 알고리즘에서는 시스템 내에서 활동하는 각각의 프로세스에 고유의 우선순위가 할당되어 있다고 가정한다.  
    - 조정자는 항상 가장 큰 우선순위 번호를 가진 프로세스이다.  
    - 선출 알고리즘의 첫 번째 알고리즘은 프로세스가 시스템 내의 모든 다른 프로세스에 메시지를 보낼 수 있는 시스템에 적용될 수 있고, 두 번쨰 알고리즘은 논리적 또는 물리적으로 구성된 링 구조의 시스템에 적용될 수 있다.  
    - Bully 알고리즘: 프로세스 Pi 는 자신보다 높은 우선순위를 가진 모든 프로세스에 선출 메시지를 보낸다. 일정 시간 동안 아무런 응답이 도착하지 않으면 i보다 우선순위가 큰 모든 프로세스에 고장이 있다고 생각하고 Pi 자신을 새로운 조정자로 선출한다.  
    조정자의 기능을 자신에게 이전시키고, 프로세스 Pi 가 새로운 조정자라는 메시지를 i보다 우선순위가 낮은 모든 프로세스에 보낸다.  
    일정 시간 내에 응답을 받게 되면, Pi 는 자신보다 높은 우선순위를 가진 프로세스가 선출되었다는 메시지를 일정 시간 동안 기다린다.  
    만약 일정 시간 내에 선출되었다는 메시지를 받지 못하면 위 과정을 다시 실행한다.  
    고장난 프로세스가 복구되면 같은 알고리즘을 실행한다. 자신보다 높은 순위 번호의 프로세스가 없으면 현재 낮은 순위의 조정자가 있다고 하더라도 자신이 프로세스 조정자가 된다는 사실을 알려주여야 한다.  
    - 링 알고리즘: 링 알고리즘에서 링크는 단방향이며, 사용되는 주요 데이터 구조는 활동 리스트이다. 이 리스트는 알고리즘이 끝날 때 시스템 내의 활동하는 모든 프로세스의 우선순위 번호를 가진다. 각 프로세스는 자신의 활동 리스트를 유지한다.  
    프로세스 Pi 가 조정자의 고장을 탐지하면 비어 있는 새로운 활동 리스트를 만든다. 오른쪽 프로세스에 elect(i) 메시지를 보내고 활동 리스트에 i 를 추가한다.  
    Pi 가 자신의 왼쪽 프로세스로부터 elect(j) 란 메시지를 받으면 다음 세가지 중 하나로 응답해야 한다.  
    1) 만약 첫 번째 elect 메시지라면 Pi 는 i 와 j 번호를 갖는 새로운 활동 리스트를 만든다. 그리고 elect(j) 와 elect(i) 메시지를 차례대로 전송한다.  
    2) i =/= j 라면, Pi 는 j 를 활동리스트에 더하고 오른쪽 프로세스에 메시지를 전달한다.  
    3) i = j 라면, Pi 의 활동 리스트가 시스템 내의 활동하는 모든 프로세스의 순위 번호를 포함하고 있음을 의미한다. 프로세스 Pi는 활동 리스트에서 가장 큰 순위 번호를 갖는 새로운 조정자 프로세스를 선출한다.  
    - 링 알고리즘은 고장난 프로세스가 복구되었을 때 현재 조정자 프로세스의 순위를 결정하는 방법을 규정하지 않는다.  
    - 시스템이 신뢰성을 갖기 위해 프로세스 집합이 어떤 공통값에 합의하도록 하는 기법이 필요하다.  
    이런 의견 불일치가 일어나는 데에는 여러 이유가 있다. 첫 째로 통신 매체 고장으로 메시지를 잃어버리거나 잘못 전달될 수 있다. 둘 째로 프로세스 고장으로 예측할 수 없는 행위가 유발될 수 있다.  
    최선은 프로세스가 그 실행을 멈추는 것이고, 최악은 프로세스가 잘못된 메시지를 전달해 시스템 무결성이 파괴되는 것이다.  
    - 이러한 문제는 비잔틴 장군 문제로 생각할 수 있다.  
    - 프로세스가 고장일 경우 시스템 무결성을 해치지 않으며 통신매체가 신뢰할 수 없다고 가정하자. 이 경우 고장을 탐지하기 위하여 타임아웃 기법을 사용할 수 있다.  
    - 통신 매체가 신뢰할 수 있다고 가정하여도 프로세스는 예측할 수 없는 형태로 고장날 수 있다. n 개의 프로세스 중 m 개 이하의 프로세스가 고장난 시스템이 있을 때 각 프로세스 Pi 가 사유의 값 Vi 를 가졌다고 가정하자.  
    고장나지 않은 프로세스 Pi 가 다음을 만족하는 벡터 Xi = (Ai1, Ai2, ..., Ain) 을 구성할 수 있는 알고리즘이 필요하다.  
    1) Pj 가 고장나지 않았으면 Aij = Vj 이다.  
    2) Pi 와 Pj 가 둘 다 고장나지 않았으면 Xi = Xj 이다.  
    이 문제에 대해 많은 해결 방법이 있는데 이들 해결 방법은 아래의 특성을 공통적으로 갖고 있다.  
    1) n >= 3 * m + 1 인 경우에만 올바른 알고리즘이 고안될 수 있다.  
    2) 최악의 경우 합의에 도달하는 데 걸리는 지연은 m + 1 의 메시지 전송의 지연과 비례한다.  
    3) 의견이 일치할 때까지 필요한 메시지 수가 많다. 모든 프로세스로부터 정보를 모아 결정해야 하기 때문이다.  
    
#### 19장) 실시간 시스템
    - 실시간 시스템은 특정한 마감시간 이내에 정확한 컴퓨팅 결과를 생성해낼 수 있는 컴퓨팅 시스템을 말한다.  
    - 어떤 실시간 시스템은 안전 필수(Safety Critical) 시스템이라고 불리기도 한다. 안전 필수 시스템은 마감 시간을 어기는 것과 같은 잘못된 연산이 큰 문제를 일으킬 수 있는 시스템을 말한다.  
    안전 필수 시스템의 예로 무기 시스템, 브레이크 시스템, 비행기 제어 시스템, 심장 박동 조절장치 등이 있다.  
    - 실시간 컴퓨팅은 경성 타입과 연성 타입이 있다. 경성 실시간 시스템은 실시간 태스크는 마감 시간 안에 반드시 완수해야 하는 가장 엄격한 제약 조건을 갖고 있다.  
    안전 필수 시스템은 전형적인 경성 실시간 시스템이다.  
    - Linux 와 같은 많은 운영체제들이 연성 실시간 기능을 제공한다.  
    - 실시간 시스템은 전형적으로 하나의 목적, 작은 사이즈, 적은 비용으로 양산, 시간 제약의 특징을 갖고 있다.  
    - 실시간 시스템은 가전제품이나 디지털 장치 등에서 많이 사용되고 있다.  
    - 많은 임베디드 시스템 제어기의 경우 SOC(System On Chip) 전략을 사용한다. 주변 기기의 기능이 모두 하나의 칩에 있는 것을 말한다.  
    - 실시간 시스템은 한 가지 목적을 위해 만들어졌기 때문에 데스크톱 PC 가 갖고 있는 많은 특징들을 필요로 하지 않는다.  
    - 일반적으로 실시간 시스템은 사용자 개념을 갖고 있지 않다.  
    - 실시간 운영체제에서 논리 메모리 - 가상 메모리 맵핑 주소 변환하는 첫 번째 방법은 논리 주소를 우회하여 CPI가 직접 물리 주소로 변환하는 것이다. 실주소 모드(Real addressing Mode)로 알려져 있다.  
    이 방법의 문제점은 프로세스 간의 메모리 보호 기법이 없다는 것이다. 또한 프로그래머가 물리적 위치를 명시해야 한다. 반면 매우 빠르다.  
    실주소 모드는 일반적으로 경성 실시간 시스템에서 많이 볼 수 있다.  
    - 주소를 변환하는 두 번째 방법은 재배치 레지스터 R에 프로그램이 적재될 메모리 위치가 적재되고, 물리주소 P = R + L 로 정의하는 방식이다.  
    이 방법 역시 MMU 가 P = R + L 을 함으로써 매우 쉽게 주소 변환을 할 수 있는 반면 프로세스 간 메모리 보호가 제공되지 않는다.  
    - 주소를 변화하는 세 번째 방법은 페이지 테이블과 TLB 를 사용하는 것이다. 이 경우 프로그램이 메모리 위치에 상관없이 적재될 수 있으며 메모리 보호가 제공된다.  
    - 실시간 운영체제에서 가장 중요한 기능은 실시간 프로세스가 CPU를 필요로 할 때 바로 응답하는 것이다. 따라서 스케쥴러는 선점을 이용한 우선순위 기반의 알고리즘을 지원해야 한다.  
    - 선점 및 우선순위 기반의 스케쥴러를 제공하는 것은 단지 연성 실시간 기능을 제공하는 것에 불과하다.  
    - 경성 실시간 시스템에서는 실시간 태스크가 마감시간 내에 확실히 실행되는 것이 보장되어야 하며 그렇기에 부가적인 스케쥴링 기법이 필요하다.  
    - 실시간 시스템, 특히 경성 시스템의 시간 조건을 만족시키기 위해서 선점형 커널이 필수적이다.  
    - 선점형 커널을 만들기 위한 한 가지 방법으로 실행시간이 긴 시스템 호줄의 경우 선점 포인트를 두는 것이다. 선점 포인트 때마다 실행을 기다리는 높은 우선순위의 프로세스가 있는지 체크해서 존재할 경우 문맥교환을 실행한다.  
    - 인터럽트 지연시간과 디스패치 지연시간이 실시간 시스템의 성능을 좌우한다.  
    - 인터럽트 지연시간에 영향을 주는 요인 중 하나는 커널 데이터 구조체를 갱신하는 동안 인터럽트가 불능케 되는 시간이다.  
    - 디스패치 지연시간은 스케쥴링 디스패처가 하나의 프로세스를 봉쇄시키고 다른 프로세스를 시작하게 하는데 걸리는 시간을 말한다.  
    - 디스패치 지연시간의 충돌 단계는 아래 두 가지 요소로 구성된다.  
    1) 커널에서 동작한느 프로세스에 대한 선점  
    2) 높은 우선순위의 프로세스가 필요한 자원을 낮은 우선순위의 프로세스 자원이 방출  
    - 우선순위 역전이라고 알려진 문제는 우선순위 상속 프로토콜을 사용하여 해결할 수 있다.  
    - Rate Monotonic 스케쥴링 알고리즘은 선점 가능한 정적 우선순위 정책을 이용하여 주기 태스크들을 스케쥴한다.  
    주기가 짧은 태스크는 높은 우선순위가, 주기가 길면 낮은 우선순위가 배정된다.  
    - EDF(Earliest Deadline First) 스케쥴링에서 태스크는 마감시간이 빠를수록 우선순위가 높아지고 늦을수록 낮아진다.  
    - 비례 지분(Proportional Share) 스케쥴링에서는 모든 프로세스가 T 개의 시간 지분을 적절히 나눠가져 스케쥴링된다.  
    - POSIX 는 실시간 컴퓨팅 용으로 POSIX.1b 라는 확장을 제공한다.  
    - 경성 실시간 시스템을 제공하는 유명한 실시간 운영체제로 VxWorks 가 있다.  
    
#### 20장) 멀티미디어 시스템
    - 멀티미디어 데이터는 기타 다른 데이터와 마찬가지로 파일 시스템에 저장된다. 일반 파일과의 차이점은 멀티미디어 파일은 지정된 속도로 접근되어야 하는 반면 일반 파일에 접근하는 것은 특별한 타이밍에 대한 요구사항이 없다는 것이다.  
    - 속도에 대한 요구사항이 연관된 데이터를 연속 매체 데이터라고 한다.  
    - 스트리밍 기법에는 점진적 다운로드와 실시간 스트리밍의 두 가지가 있다.  
    - 점진적 다운로드에서 오디오 또는 비디오를 포함한 데이터가 클라이언트의 지역 파일 시스템에 저장된다.  
    - 실시간 스트리밍은 매체 파일이 클라이언트에 스트림되지만 저장되지 않는다.  
    - 점진적 다운로드와 실시간 스트리밍에서 모두 클라이언트가 스트림의 다른 위치로 이동하는 것이 가능하다. 스트림 내에서 이동할 수 있는 기능은 임의 접근이라고 알려져 있다.  
    - 실시간 스트리밍에는 라이브 스트리밍과 요구형 스트리밍 두 종류가 있다.  
    - 라이브 스트리밍은 이벤트가 실제로 발생하는 시점에 생중계로 전송할 때 사용한다.  
    - 요구형 스트리밍은 사건이 일어나는 시점에 발생하지 않는다.  
    - 멀티미디어 시스템은 다음과 같은 특성을 갖는다.  
    1) 멀티미디어 파일은 크다.  
    2) 연속 매체는 높은 데이터율을 필요올 한다.  
    3) 멀티미디어 응용은 재생 중 시간 지연에 민감하다.  
    - 컴퓨터 시스템이 연속 매체 데이터를 전송하기 위해서 연속 매체의 특정 속도와 시간적 요구사항(QoS 요구사항, 서비스 품질)을 보장해야 한다.  
    - QoS 를 만족시키기 위해 다음과 같은 것이 고려되어야 한다.  
    1) 압축과 해제는 상당량의 CPU 처리를 요하므로 고려되어야 한다.  
    2) 멀티미디어 태스크는 연속 매체의 마감 시간 요구를 만족시키기 위해 우선순위로 스케쥴링되어야 한다.  
    3) 파일시스템 역시 효율적이어야 한다.  
    - 멀티미디어 시스템의 크기와 속도의 요구사항으로 인해 멀티미디어 파일들은 종종 원래 형태보다 작은 형태로 압축된다.  
    - 압축 알고리즘은 손실 알고리즘과 비손실 알고리즘으로 나뉜다.  
    - 손실 압축을 사용하면 압축 해제 시 원본 데이터의 일부가 손실된다.  
    - 비손실 압축을 사용하면 압축된 파일이 항상 원래의 형태로 복원될 수 있음을 보장한다.  
    - 손실 압축은 이미지, 오디오, 비디오와 같은 특정 종류의 데이터에서만 사용할 수 있다.  
    - 비디오 파일에 사용되는 몇몇 손실 압축 알고리즘은 단지 연속된 프레임의 차이점만을 저장하는 방법으로 동작한다.  
    - 비손실 알고리즘은 컴퓨터 프로그램과 같은 텍스트 파일을 압축하는 데에 사용된다.  
    - MPEG 는 디지털 비디오를 위한 파일 형식과 압축 표준의 집합을 지칭한다. 디지털 비디오는 통산 오디오 부분까지 포함하기 때문에 각 표준은 세 계층으로 나뉜다.  
    - 계층 3 과 계층 2 는 매체 파일의 오디오와 비디오 부분에 적용된다.  
    - 시스템 계층이라 불리는 계층 1은 타이밍 정보를 포함하고 있으며 이는 MPEG 플레이어가 오디오와 비디오 부분을 다중화(Multiplex) 하여 재생 중 이들을 동기화시키는 것을 가능토록 한다.  
    - 주요 MPEG 표준으로 MPEG-1, MPEG-2, MPEG-4 등이 있다.  
    - MP3 오디오 파일은 MPEG-1 의 오디오 계층(계층 3) 을 사욯한다.  
    - 위 세 MPEG 표준은 모두 높은 압축률을 얻기 위하여 손실 압축을 실행한다.  
    - 일정한 간격으로 데이터를 요청하는 태스크를 주기 프로세스라고 한다.  
    - QoS 는 아래 세 가지 수준으로 나뉜다.  
    1) 최선(Best effort) 서비스: 시스템은 요구사항을 만족시키기 위해 최선을 다하나 요구사항을 만족시킨다는 보장은 없다.  
    2) 소프트 QoS: 특정 트래픽 스트림에 다른 스트림보다 높은 우선순위를 부여함으로써 여러 종류의 트래픽을 상이한 방법으로 처리한다.  
    3) 하드 QoS: 서비스 품질 요구사항이 보장된다.  
    - 일반적인 운영체제는 최선 서비스만을 제공하며 초과공급에 의존한다. 즉 가용한 자원의 전체 양이 작업에 필요로 하는 자원 양보다 크다고 가정하는 것이다.  
    - 멀티미디어 응용에 대한 QoS 를 정의하는 다수의 매개변수가 존재한다. 매개변수에는 다음과 같은 것이 포함된다.  
    1) 처리량(Throughout): 특정 기간 동안 완료된 작업의 총량을 말한다. 멀티미디어 응요에서 처리량은 요구된 데이터 전송 속도이다.  
    2) 지연: 요청이 처음으로 제출된 시점부터 요구된 결과가 산출된 시점까지 소요된 시간이다.  
    3) 지터(jitter): 지터는 지연과 연관이 있다. 지터는 스트림의 재생 도중 발생하는 지연을 의미한다. 통상적으로 클라이언트는 재생이 시작되기 전에 특정 시간 동안 데이터를 버퍼링함으로써 지터를 보정할 수 있다.  
    4) 신뢰성: 전송 중이나 연속 매체 처리 도중 발생하는 오류를 어떻게 처리하는 지를 말한다.  
    - 운영체제는 QoS 를 보장하기 위해 보통 승인 제어를 사용하는데, 이는 단순하게 서버가 요청을 만족시키기에 충분한 자원을 갖고 있는 경우에 요청을 승이하는 것이다.  
    - 승인 제어를 구현하는 일반적인 기법은 자원 예약을 사용하는 것이다.  
    - 연성 실시간 시스템은 단순히 긴급 프로세스에 스케쥴링 우선순위를 부여한다. 긴급 프로세스가 우선권을 갖지만 언제 스케쥴될 지에 대해서는 보장하지 못한다.  
    - 실시간 태새크를 스케쥴링할 때에는 대부분 시스템이 정적 우선순위를 부여하는데 이는 스케쥴려 설계가 덜 복잡하기 때문이다.  
    - EDF(Earliest Deadline First) 스케쥴링은 디스크 스케쥴링 알고리즘으로 사용될 수 있으며, 이 경우 EDF 는 큐를 사용하여 각 요청이 완료되어야 하는 시간에 따라 요청들의 순서를 정한다.  
    이 접근 방법의 문제점은 엄격하게 마감 시간에 따라 요청을 처리함으로써 탐색 시간이 높아질 수 있다는 것이다.  
    - 디스크 스케쥴링에 SCAN-EDF 알고리즘을 사용할 수 있다. SCAN-EDF 알고리즘은 EDF 순서에 따라 시작하지만 같은 마감시간을 가진 요청들을 SCAN 순서에 따라 처리한다.  
    만약 상이한 마감시간을 가진 여러 요청이 인접해있다면 요청들을 묶어서 같은 묶음에 속한 요청들을 SCAN 순서에 따라 처리한다. 유사한 마감시간을 가진 요청들을 묶는 데에는 여러 기법이 존재한다.  
    - 일반적인 컴퓨터 네트워크는 요구된 시간 내에 데이터 전송을 보장하는 네트워크 프로토콜을 제공하지 않는다.  
    - 타이밍 문제를 해결하는 한 가지 프로토콜은 실시간 전송 프로토콜(RTP, Real Time transport Protocol) 이다. RTP 는 오디오와 비디오를 포함한 실시간 데이터를 전송하기 위한 인터넷 표준이다.  
    - 일반적으로 네트워크를 통해 서버에서 클라이언트로 콘텐츠를 전송하는 방법으로 세 가지가 있다.  
    1) 유니캐스팅: 서버는 단일 클라이언트에 콘텐츠를 전송한다. 하나 이상의 클라이언트에 콘텐츠가 전송되는 경우 서버가 각 클라이언트와 개별적인 유니캐스트를 구축해야 한다.  
    2) 브로드캐스팅: 서버는 클라이언트가 콘텐츠를 수신하기를 원하는지 여부에 관계없이 모든 클라이언트에게 콘텐츠를 전송한다.  
    3) 멀티캐스팅: 서버는 콘텐츠 수신을 원한다고 표명한 일단의 수신자에게 콘텐츠를 제공한다. 유니캐스팅과 브로드캐스팅의 사이에 위치한다.  
    - 유니캐스팅의 문제는 서버가 각 클라이언트를 위해 개별적인 세션을 구축해야 한다는 것이다. 라이브 실시간 스트리밍의 경우 서버가 동일한 콘텐츠의 복사본을 각 클라이언트마다 하나씩 만들어야 하기 때문에 특히 비효율적이다.  
    - 모든 클라이언트가 수신하기를 원하는 것이 아닐 수 있으므로 브로드캐스팅이 항상 적절한 것은 아니다.  
    - 만일 서버로부터 클라이언트에 도달하는 경로가 중계 라우터를 거쳐야 한다면 라우터 역시 멀티캐스팅을 지원해야 한다.  
    - 현재 대부분의 스트리밍 매체는 유니캐스팅을 통해 전송된다.  
    - 표준 웹 서버로부터 스트리밍 매체를 전송할 때 문제는 HTTP는 무상태(stateless) 프로토콜로 간주된다는 것이다.  
    - 스트리밍 매체를 위해 특별히 설계된 스트리밍 서버를 사욯할 수 있다. 스트리밍 서버와 매체 재생기 사이의 통신을 위해 설계된 한 가지 프로토콜은 실시간 스트리밍 프로토콜(RTSP, Real Time Streaming Protocol) 로 알려져 있다.  
    RTSP 가 제공하는 중요한 장점은 클라이언트와 서버 간의 상태 연결이다. 이로 인해 클라이언트는 재생 도중 일시정지하거나 스트림의 임의의 위치로 이동하는 것이 가능하다.  
    
#### 21장) Linux 시스템
    - 차후 실제 코드를 보며 익히기 위해 스킵한다.  

#### 22장) 윈도우즈 XP
    - 차후 실제 코드를 보며 익히기 위해 스킵한다.(볼 수 있나?)  
...
이렇게 운영체제 리뷰를 마무리한다.  
겉핥기 식으로 하여 깊이가 부족하겠지만 알게 된게 어디인가 싶다.  
이 책을 다 읽고 나서 리눅스 커널을 보며 실제 코드와 친해져봐야겠다는 생각에 다른 책도 샀다.  
새로 산 책은 교과서라기보다는 초보용 커널 탐구 지침서 같은 느낌인데, 천천히 시작을 해보겠다.  

아무튼 책을 보면 유사한 알고리즘이 군데군데에 튀어나온다.  
정책과 매커니즘을 분리하여 매커니즘을 일반화했기 때문이 아닐까 하는 생각이 들었다.  

이런 것을 생각하면서 일할 수 있을까.. 싶긴 한데 어떻게든 도움이 될 것이다..
